/**
 * dbo.signatory — a locator's authorized signatories (legacy BRIDGE dbSignatory).
 *
 * Child table of dbo.proponents, edited from the locator form (under the "Contact Person" tab)
 * and synced by id on every locator save: rows are updated or inserted, and rows removed in
...
/** A locator's active signatories, oldest first. */
...
/** Syncs a locator's signatories to the submitted list, inside the caller's transaction:
...
async function syncForProponent(tx, proponentId, rows, actorId) {
  ...
  const existing = await tx.query(`SELECT id FROM dbo.signatory WHERE proponent_id = @param0 AND is_active = 1`, [proponentId]);
  const keep = new Set();
  ...
  for (const row of incoming) {
    ...
    if (owned) {
      ...
      await tx.query(
        `INSERT INTO dbo.signatory (proponent_id, name, designation, contact_no, email, created_by)
  ...
  for (const e of existing.recordset || []) {
    if (!keep.has(e.id)) {
      await tx.query(`UPDATE dbo.signatory SET is_active = 0, updated_by = @param1, updated_at = GETDATE() WHERE id = @param0`, [e.id, actor]);
    }
  }
}
