 ...
 * memo so the next call retries. (No seed data: the legacy dbLandUse only holds two placeholder rows, "TESTING" / "ADAFS".) */
...
async function listLandUses() {
  await ensureSchema();
  const rows = await selectData(`SELECT ${COLUMNS} FROM dbo.land_use x ORDER BY x.name ASC`);
  return rows.map(mapRow);
}

async function getLandUseById(id) {
  await ensureSchema();
  const rows = await selectData(`SELECT TOP (1) ${COLUMNS} FROM dbo.land_use x WHERE x.id = @param0`, [id]);
  const row = rows?.[0];
  return row ? mapRow(row) : null;
}

async function createLandUse({ name, created_by, is_active = 1 }) {
  await ensureSchema();
  const result = await insertData(
    `
    INSERT INTO dbo.land_use (name, created_by, updated_by, created_at, updated_at, is_active)
    OUTPUT INSERTED.id
    VALUES (@param0, @param1, NULL, GETDATE(), NULL, @param2)
    `,
    [name, toInt(created_by), is_active ? 1 : 0]
  );
  return getLandUseById(result?.recordset?.[0]?.id);
}

async function updateLandUse(id, { name, is_active, updated_by }) {
  await ensureSchema();
  const sets = [];
  const params = [];
  ...
  return getLandUseById(id);
}

async function setLandUseActive(id, active, updated_by) {
  await ensureSchema();
  await updateData(
    `
    UPDATE dbo.land_use
...
const deactivateLandUse = (id, updated_by) => setLandUseActive(id, false, updated_by);
const reactivateLandUse = (id, updated_by) => setLandUseActive(id, true, updated_by);
