...
- Lockout: a wrong password or a wrong authenticator code counts toward `LOGIN_MAX_ATTEMPTS` (default 5, then locked for `LOGIN_LOCKOUT_MINUTES`, default 15). Admins are exempt. The counter clears only after a full sign-in, so a correct password alone doesn't reset it. Logout is `POST /api/auth/logout` only.
