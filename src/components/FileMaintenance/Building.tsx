...
const MENU_KEY = 'settings:building';
...
export function BuildingManagement() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(json?.message || 'Failed to load building');
  ...
  function openEdit(item: BuildingRow) {
    setEditing(item);
  ...
  async function save() {
    ...
    try {
      ...
      const res = await fetch(editing ? `/api/building/${editing.id}` : '/api/building', {
      ...
      await loadAll();
      toast.success(editing ? 'Building updated successfully' : 'Building created successfully');
    } catch (e: any) {
  ...
  async function setActive(id: number, active: boolean) {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/building/${id}/${active ? 'reactivate' : 'deactivate'}`, {
        method: 'PATCH',
        credentials: 'include',
      });
      const json = await res.json().catch(() => ({}));
      ...
      await loadAll();
      toast.success(active ? 'Building reactivated successfully' : 'Building deactivated successfully');
    } catch (e: any) {
  ...
  function renderStatusBadge(item: BuildingRow) {
    return (
      <span
  ...
  function renderItemActions(item: BuildingRow) {
    return (
      <>
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      <div className="grid grid-cols-3 gap-2 sm:gap-4 mt-3">
        <StatCard label="Active Buildings" value={String(stats.active)} />
        <StatCard label="Total Buildings" value={String(stats.total)} />
          ...
          <EmptyState
            ...
            description={
              searchQuery
                ...
                : 'There are no buildings yet. Create one to get started.'
            }
            action={
              !searchQuery && canAdd ? (
                <button
      ...
      <SidePanel
        ...
        subtitle="Building master table"
      ...
      <ConfirmModal
        ...
        description="This building will be marked inactive. You can re-activate later."
      ...
      <ConfirmModal
        ...
        description="This building will be marked active again."
