...
export function ApplicationTypesManagement() {
  ...
  const canSubmit = useMemo(() => {
    ...
    if (!editing) return Boolean(code || name || description);
    ...
    <div className="space-y-4 sm:space-y-5">
      ...
      <div className="glass-card p-3 sm:p-5 !border-transparent" style={{ backgroundColor: 'var(--surface)' }}>
        <div className="flex items-center justify-between mb-3 gap-2">
          ...
          {canAdd ? (
            <button
              className="shrink-0 whitespace-nowrap inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-[11px] font-semibold shadow-sm"
        ...
        {loading ? (
          ...
          <div>
            ...
            {/* Phones: one card per application type instead of a 5-column table */}
      ...
      >
        <div className="grid grid-cols-1 gap-3">
          <Field label="Code">
            <input
              className="app-input"
              style={{ opacity: editing ? 0.6 : 1, cursor: editing ? 'not-allowed' : 'text' }}
              value={form.code}
