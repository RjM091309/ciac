...
export function LandUseManagement() {
  ...
  const [items, setItems] = useState<LandUseRow[]>([]);
  const [editing, setEditing] = useState<LandUseRow | null>(null);
  ...
  async function loadAll() {
    ...
    setError(null);
    try {
      const res = await fetch('/api/land-use', { credentials: 'include' });
      const json = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(json?.message || 'Failed to load land use');
  ...
  function openEdit(item: LandUseRow) {
    setEditing(item);
  ...
  async function save() {
    ...
    try {
      ...
      const res = await fetch(editing ? `/api/land-use/${editing.id}` : '/api/land-use', {
      ...
      await loadAll();
      toast.success(editing ? 'Land Use updated successfully' : 'Land Use created successfully');
    } catch (e: any) {
  ...
  async function setActive(id: number, active: boolean) {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/land-use/${id}/${active ? 'reactivate' : 'deactivate'}`, {
        method: 'PATCH',
        credentials: 'include',
      });
      const json = await res.json().catch(() => ({}));
      ...
      await loadAll();
      toast.success(active ? 'Land Use reactivated successfully' : 'Land Use deactivated successfully');
    } catch (e: any) {
  ...
  function renderStatusBadge(item: LandUseRow) {
    return (
      <span
  ...
  function renderItemActions(item: LandUseRow) {
    return (
      <>
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      <div className="grid grid-cols-3 gap-2 sm:gap-4 mt-3">
        <StatCard label="Active Land Uses" value={String(stats.active)} />
        <StatCard label="Total Land Uses" value={String(stats.total)} />
      ...
      <div className="glass-card p-3 sm:p-5 !border-transparent" style={{ backgroundColor: 'var(--surface)' }}>
        ...
        {loading ? (
          ...
          <EmptyState
            title="No land use found"
            description={
              searchQuery
                ? 'Try adjusting your search filters.'
                : 'There are no land uses yet. Create one to get started.'
            }
            action={
              !searchQuery && canAdd ? (
                <button
      ...
      <SidePanel
        open={isPanelOpen}
        title={editing ? 'Edit Land Use' : 'New Land Use'}
        subtitle="Land Use master table"
      ...
      <ConfirmModal
        ...
        title="Deactivate land use?"
        description="This land use will be marked inactive. You can re-activate later."
      ...
      <ConfirmModal
        ...
        title="Reactivate land use?"
        description="This land use will be marked active again."
