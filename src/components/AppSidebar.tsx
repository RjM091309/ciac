...
export function AppSidebar({
  ...
  const showSystemSettings =
    canView('settings:users') || canView('settings:checklist') || canView('settings:control-panel') || canView('settings:audit-log');
  ...
  const permitsItems: SidebarLeaf[] = !canView('compliance:permits') ? [] : [
    { key: 'compliance:permits-1', label: 'Environmental, Fire, Occupancy, Sanitary', active: view === 'compliance:permits', onClick: () => onViewChange('compliance:permits') },
    { key: 'compliance:permits-2', label: 'Authority to Operate', active: view === 'compliance:permits', onClick: () => onViewChange('compliance:permits') },
  ];

  const birTaxItems: SidebarLeaf[] = !canView('compliance:bir') ? [] : [
    { key: 'compliance:bir-1', label: 'BIR Tax Clearance', active: view === 'compliance:bir', onClick: () => onViewChange('compliance:bir') },
    { key: 'compliance:bir-2', label: 'Certificate of Registration', active: view === 'compliance:bir', onClick: () => onViewChange('compliance:bir') },
    { key: 'compliance:bir-3', label: 'Receipts / Invoices / ATP', active: view === 'compliance:bir', onClick: () => onViewChange('compliance:bir') },
    { key: 'compliance:bir-4', label: 'POS / CRM Permit', active: view === 'compliance:bir', onClick: () => onViewChange('compliance:bir') },
  ];
  ...
  const systemSettingsItems: SidebarLeaf[] = [
    ...
    canView('settings:checklist') && { key: 'settings:checklist', label: 'Master Checklist', active: view === 'settings:checklist', onClick: () => onViewChange('settings:checklist') },
