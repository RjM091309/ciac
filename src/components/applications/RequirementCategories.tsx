  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      ...
      <div className="flex items-center justify-between gap-2">
        <h3 className="min-w-0 text-sm sm:text-lg font-bold tracking-tight" style={{ color: 'var(--text)' }}>
          Requirement Categories List
        </h3>
        {canAdd ? (
          <button
            className="shrink-0 whitespace-nowrap rounded-lg px-3 py-2 text-xs sm:text-sm font-semibold inline-flex items-center gap-1.5 shadow-sm cursor-pointer"
      ...
      <div className="glass-card p-3 sm:p-5 !border-transparent overflow-hidden" style={{ backgroundColor: 'var(--surface)' }}>
        ...
        {isLoading ? (
          ...
          <EmptyState
            title="No requirement categories found"
            description={
              searchQuery
                ? 'Try adjusting your search filters.'
                : 'There are no requirement categories to show here yet. Create a new category to get started.'
            }
            action={
              !searchQuery && canAdd ? (
                <button
          ...
          <>
          {/* Phones: one card per category instead of a 4-column table */}
